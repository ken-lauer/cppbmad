...
struct FortranTraits {
  ...
  static constexpr std::string_view type_name() { return "Unknown"; }
};
...
struct has_fortran_traits<
    ...
    std::void_t<
        ...
        decltype(FortranTraits<T>::copy(std::declval<const void *>(), std::declval<void *>()))>>
    : std::true_type {};
...
template <typename T>
class FortranProxy {
  static_assert(has_fortran_traits_v<T>, "Type T must have specialized FortranTraits");
...
public:
  ...
  FortranProxy()
      : fortran_ptr_(FortranTraits<T>::allocate())
      , owns_memory_(true) {
    if (!fortran_ptr_) {
      throw std::bad_alloc();
    }
  ...
  ~FortranProxy() { cleanup(); }

  // TODO: pointer-to-a-pointer for fortran-side `type(c_ptr), value`
  void *get_fortran_ptr() const noexcept { return fortran_ptr_; }

  bool owns_memory() const noexcept { return owns_memory_; }

  bool is_valid() const noexcept { return fortran_ptr_ != nullptr; }
