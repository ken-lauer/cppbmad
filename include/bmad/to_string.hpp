    ...
    auto print_recursive = [&](auto &&self, size_t current_dim) -> void {
      ...
      for (int i = 0; i < dim_size; ++i) {
        ...
        if (current_dim == N - 1) {
          ...
          oss << to_string(detail::farray_get_packed(arr, indices, std::make_index_sequence<N>{}));
        } else {
...
std::string to_string(const FTypeArrayND<ProxyType, N, AllocFunc, DeallocFunc> &arr) {
  std::ostringstream oss;
  oss << "[";
  if constexpr (N == 1) {
