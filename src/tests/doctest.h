...
struct ContextOptions //! OCLINT too many fields
{
  ...
  String strip_file_prefixes; // remove the longest matching one of these prefixes from any file
                              // paths in the output
...
struct StringMaker : public detail::StringMakerBase<
                         ...
                         detail::types::is_pointer<T>::value || detail::types::is_array<T>::value> {
};
...
template <
    typename T,
    typename detail::types::
        enable_if<!detail::should_stringify_as_underlying_type<T>::value, bool>::type = true>
...
template <
    typename T,
    typename detail::types::enable_if<detail::should_stringify_as_underlying_type<T>::value, bool>::
        type = true>
String toString(const DOCTEST_REF_WRAP(T) value) {
...
String
stringifyBinaryExpr(const DOCTEST_REF_WRAP(L) lhs, const char *op, const DOCTEST_REF_WRAP(R) rhs) {
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
  ...
  template <
      typename L,
      typename types::enable_if<!doctest::detail::types::is_rvalue_reference<L>::value, void>::type
          * = nullptr>
...
struct DOCTEST_INTERFACE TestCase : public TestCaseData {
  ...
  int m_template_id; // an ID used to distinguish between the different versions of a templated test
                     // case
...
struct DOCTEST_INTERFACE ResultBuilder : public AssertData {
  ...
  binary_assert(const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) {
...
template <typename L>
DOCTEST_NOINLINE bool unary_assert(
    assertType::Enum at,
    const char *file,
    int line,
    const char *expr,
...
struct DOCTEST_INTERFACE IReporter {
  ...
  // called when a test case is reentered because of unfinished subcases (safe to cache a pointer to
  // the input)
  ...
  // decorator or isn't in the execution range (between first and last) (safe to cache a pointer to
  // the input)
...
#define DOCTEST_TEST_CASE_FIXTURE(c, decorators) \
  DOCTEST_IMPLEMENT_FIXTURE(                     \
      DOCTEST_ANONYMOUS(DOCTEST_ANON_CLASS_),    \
      c,                                         \
      DOCTEST_ANONYMOUS(DOCTEST_ANON_FUNC_),     \
      decorators                                 \
  )
...
#define DOCTEST_ASSERT_IMPLEMENT_2(assert_type, ...)                                         \
  ...
  doctest::detail::ResultBuilder                                                             \
      DOCTEST_RB(doctest::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__);        \
...
#define DOCTEST_EXCEPTION_EMPTY_FUNC                                                         \
  [] {                                                                                       \
    static_assert(                                                                           \
        ...
        "Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS if you want to compile with " \
        "exceptions disabled."                                                               \
    );                                                                                       \
    return false;                                                                            \
  }()
  ...
  if (getContextOptions()->no_path_in_filenames) {
    ...
    for (String::size_type pos = 0U; pos < prefixes.size(); ++pos) {
      ...
      if (prefix_size > longest_match) {
        // TODO under DOCTEST_MSVC: does the comparison need strnicmp() to work with drive letter
        // capitalization?
...
struct JUnitReporter : public IReporter {
  ...
  struct JUnitTestCaseData {
    ...
    void
    addFailure(const std::string &message, const std::string &type, const std::string &details) {
...
bool parseOptionImpl(int argc, const char *const *argv, const char *pattern, String *value) {
  ...
  for (int i = argc; i > 0; --i) {
    ...
    if (temp &&
        (value || strlen(temp) == strlen(pattern))) { //! OCLINT prefer early exits and continue
